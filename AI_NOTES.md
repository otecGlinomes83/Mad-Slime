...
Вся работа (волны 1–7) закоммичена на ветке `otecGlinomes83/MadSlime` (== MadSlimeV2 + коммиты сессии) и отправлена в origin. Продолжать можно с любого компа: `git fetch && git checkout otecGlinomes83/MadSlime` (или влей её в MadSlimeV2 — история линейная, MadSlimeV2 является предком).
