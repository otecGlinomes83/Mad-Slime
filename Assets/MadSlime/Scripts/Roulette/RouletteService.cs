...
namespace Roulette
{
    public sealed class RouletteService : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                throw new InvalidOperationException(
                    $"{name}: PlayerProgress was not injected. Check that ProjectLifetimeScope registers PlayerProgress and RouletteService.");
            }

            if (_wallet == null)
            {
                throw new InvalidOperationException(
                    $"{name}: Wallet was not injected. Check that ProjectLifetimeScope registers Wallet and RouletteService.");
            }
